...
class SFTLayer1(nn.Module):
    def __init__(self):
        ...
        self.SFT_scale_conv0 = nn.Conv2d(32, 64, 3, 1, padding=1)
        self.SFT_scale_conv1 = nn.Conv2d(64, 64, 3, 1, padding=1)
        self.SFT_shift_conv0 = nn.Conv2d(32, 64, 3, 1, padding=1)
        self.SFT_shift_conv1 = nn.Conv2d(64, 64, 3, 1, padding=1)
        self.CondNet = nn.Sequential(
            nn.Conv2d(8, 16, 3, 1, padding=1),
            nn.LeakyReLU(0.1, True),
            nn.Conv2d(16, 32, 3, 1, padding=1))

    def forward(self, feature, prob_volume):
        # x[0]: fea; x[1]: cond
        prob_volume = self.CondNet(prob_volume)
        scale = self.SFT_scale_conv1(F.leaky_relu(self.SFT_scale_conv0(prob_volume), 0.1, inplace=True))
        shift = self.SFT_shift_conv1(F.leaky_relu(self.SFT_shift_conv0(prob_volume), 0.1, inplace=True))
        return feature * (scale + 1) + shift

class SFTLayer2(nn.Module):
    def __init__(self):
        super(SFTLayer2, self).__init__()
        self.SFT_scale_conv0 = nn.Conv2d(16, 32, 3, 1, padding=1)
        self.SFT_scale_conv1 = nn.Conv2d(32, 32, 3, 1, padding=1)
        self.SFT_shift_conv0 = nn.Conv2d(16, 32, 3, 1, padding=1)
        self.SFT_shift_conv1 = nn.Conv2d(32, 32, 3, 1, padding=1)
        self.CondNet = nn.Sequential(
            nn.Conv2d(8, 16, 3, 1, padding=1),
            nn.LeakyReLU(0.1, True),
...
class SFTLayer3(nn.Module):
    def __init__(self):
        ...
        self.SFT_scale_conv0 = nn.Conv2d(8, 16, 3, 1, padding=1)
        self.SFT_scale_conv1 = nn.Conv2d(16, 16, 3, 1, padding=1)
        self.SFT_shift_conv0 = nn.Conv2d(8, 16, 3, 1, padding=1)
        self.SFT_shift_conv1 = nn.Conv2d(16, 16, 3, 1, padding=1)
        self.CondNet = nn.Sequential(
