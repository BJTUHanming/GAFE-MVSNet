...
class MVS4net(nn.Module):
    def __init__(self, arch_mode="fpn", reg_net='reg2d', num_stage=4, fpn_base_channel=8, 
        ...
        self.conv2 = Conv2d(64, 32, 3, 1, padding=1)
        self.conv3 = Conv2d(33, 32, 3, 1, padding=1)
        self.conv4 = Conv2d(32, 16, 3, 1, padding=1)
        self.conv5 = Conv2d(17, 16, 3, 1, padding=1)
        self.conv6 = Conv2d(16, 8, 3, 1, padding=1)
        self.conv7 = Conv2d(9, 8, 3, 1, padding=1)

        self.inner1 = nn.Conv2d(32, 64, 1, bias=True)
        self.inner2 = nn.Conv2d(16, 32, 1, bias=True)
        self.inner3 = nn.Conv2d(8, 16, 1, bias=True)

        self.agg1 = nn.Conv2d(1, 1, 3, 1, padding=1, bias=True)
        self.agg2 = nn.Conv2d(1, 1, 3, 1, padding=1, bias=True)
        self.agg3 = nn.Conv2d(1, 1, 3, 1, padding=1, bias=True)
    ...
    def forward(self, imgs, proj_matrices, depth_values,flag):
        ...
        for stage_idx in range(self.num_stage):
            ...
            if stage_idx + 1 == 2:
                features_update_ref = self.SFT1(ref_outputs['stage1'], outputs_stage['attn_weight'])
                feature_add = F.interpolate(features_update_ref, scale_factor=2, mode="bilinear", align_corners=True) + self.inner1(ref_features_stage)
                features_SFT1 = self.conv2(feature_add)
                feature_concat = torch.cat((features_SFT1, self.agg1(F.interpolate(outputs['stage1']['depth'].unsqueeze(1), scale_factor=2, mode="bilinear", align_corners=True))), 1)
                ref_features_stage = self.conv3(feature_concat)

            if stage_idx + 1 == 3:
                features_update_ref = self.SFT2(ref_outputs['stage2'], outputs_stage['attn_weight'])
                feature_add = F.interpolate(features_update_ref, scale_factor=2, mode="bilinear", align_corners=True) + self.inner2(ref_features_stage)
                features_SFT2 = self.conv4(feature_add)
                feature_concat = torch.cat((features_SFT2, self.agg2(F.interpolate(outputs['stage2']['depth'].unsqueeze(1), scale_factor=2, mode="bilinear", align_corners=True))), 1)
                ref_features_stage = self.conv5(feature_concat)

            if stage_idx + 1 == 4:
                features_update_ref = self.SFT3(ref_outputs['stage3'], outputs_stage['attn_weight'])
                feature_add = F.interpolate(features_update_ref, scale_factor=2, mode="bilinear", align_corners=True) + self.inner3(ref_features_stage)
                features_SFT3 = self.conv6(feature_add)
                feature_concat = torch.cat((features_SFT3, self.agg3(F.interpolate(outputs['stage3']['depth'].unsqueeze(1), scale_factor=2, mode="bilinear", align_corners=True))), 1)
                ref_features_stage = self.conv7(feature_concat)
